...
func _ready() -> void:
	PlayerLoader.players_spawned.connect(room_setup)
	super._ready()

func room_setup(): #must change the camera for this room because the camera switching doesn't account for drag or smoothing
